myApp.controller('myExamManageController', function ($rootScope, $scope, services, $sce, $stateParams,$state,$http) {
    ...
    //学生导入
    services["_put_Import"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/excelImport/studentExamination', param, "POST");
    };
    //动态答题卡
    ...
    //考试  试卷的情况
    $scope.load = function(){
        services._get_papers($scope.get_param).success(function (res) {
            if(res.code==0){
                $rootScope.loadingEnd();
                
                $scope.all_papers = res.data.rows;
                $scope.get_param.total = res.data.total;
                $scope.get_param.pages = res.data.pages;
                $scope.set_arr = [];
                var now_time = '';
                angular.forEach($scope.all_papers,function(item,index){
                    // console.log(item.currentTime)
                    now_time = Date.parse(new Date(item.currentTime.replace(/\-/g,'/')))
                    if(item.status==2 || item.status=="2"){
                        $scope.set_arr.push(parseInt(Date.parse(new Date((item.paper_start).replace(/\-/g,'/')))))
                    }

                });

                if($scope.set_arr.length>0){
                    $scope.set_time = now_time;
                    $scope.time_int = setInterval(function(){
                        $scope.set_time = $scope.set_time+1000;
                        if($scope.set_arr.indexOf($scope.set_time+5000) !=-1 || $scope.set_arr.indexOf($scope.set_time+4000) !=-1 || $scope.set_arr.indexOf($scope.set_time+3000) !=-1 || $scope.set_arr.indexOf($scope.set_time+2000) !=-1 || $scope.set_arr.indexOf($scope.set_time+1000) !=-1 || $scope.set_arr.indexOf($scope.set_time) !=-1 || $scope.set_arr.indexOf($scope.set_time-1000) !=-1 || $scope.set_arr.indexOf($scope.set_time-2000) !=-1 || $scope.set_arr.indexOf($scope.set_time-3000) !=-1){
                            $scope.load();
                            clearInterval($scope.time_int);
                        }
                    },1000)
                }else{
                    clearInterval($scope.time_int);
                ...
                laypage.render({
                    elem: "pager",
                    count: $scope.get_param.total,
                    curr: $scope.get_param.pageNum || 1,
                    limit: $scope.get_param.pageSize,
                    layout: ['prev', 'page', 'next', 'skip'],
                    skip: true,
                    jump: function (resPager) {
                        if (resPager.curr != $scope.get_param.pageNum) {
                            $scope.get_param.pageNum = parseInt(resPager.curr);
                            $scope.load();
                        }
                    }
                });
    ...
    $scope.load();
    //试卷状态
    $scope.paper_status = ["未发布","待考试","考试中","考试结束","成绩导入中","导入失败","导入成功","待考试",'待考试','待考试'];
    //发布试卷
        ...
        services._pub_papers({id:data.id,status:data.status}).success(function (res) {
            if(res.code==0){
                ...
                layer.open({
                    ...
                    ,btnAlign: 'c' //按钮居中
                    ,shade: 0 ,//不显示遮罩
                    content:  '<div style="padding:20px 20px;text-align:center;font-size:16px;">'+ res.message +'</div>'
                });
            }
    ...
    //跳转 考试分析
    $scope.exam_lis = function(data){
        if(data.status==5){
            layer.msg("成绩还在导入中，请稍后")
            return false;
        }
        $state.go("examAnalisis",{exam_id:data.id});
    };
    //导入
    ...
    $scope.import_list = function (item){
        ...
        $('#exam-import').click();
        $("input[name='paper_id']").val(item.id)


    }

    $('#exam-import').prettyFileExcl({
        text:null,
    ...
    $scope.down_get = function(){
        ...
        if($scope.now_index==2 && $scope.down_paper==1){
            if($scope.now_examAll.source!=2){
                ...
                //     var now_name = "zonghe";
                // }
                var src_name = ConvertPinyin($rootScope._get_curriculum_name($scope.now_examAll.paper_curriculum));
                var now_name = ConvertPinyin($rootScope._get_curriculum_name($scope.now_examAll.paper_curriculum));

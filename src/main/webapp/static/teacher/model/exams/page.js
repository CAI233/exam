myApp.controller('examsController', function ($rootScope, $scope, services, $sce, $stateParams) {
    ...
    services["_del_test"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/subject/v2DelSubject', param, "POST");
    };
    ...
    $scope.delSub = function (data) {
        layer.confirm('删除后试题后将无法找回,确认删除吗？', {
           ...
           services._del_test({subject_id:data.id}).success(function (res) {
                if(res.code==0){

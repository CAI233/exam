var row_update;
myApp.controller('allPapersController', function ($rootScope, $scope, services, $sce, $stateParams, $state){
    ...
    del_Paper = function(id){
        layer.confirm('删除试卷后将无法找回,确认删除吗？', {
            ...
            services._del_testList({id:id,token:$rootScope.token}).success(function (res) {
                if(res.code==0){
    ...
    $scope.getPaperDuration = function () {
        if ($scope.selRow.paper_end && $scope.selRow.paper_start) {
            var tt = Date.parse(new Date($scope.selRow.paper_end)) - Date.parse(new Date($scope.selRow.paper_start))

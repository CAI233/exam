...
myApp.controller('userController', function ($rootScope, $scope, services, $sce, $stateParams) {
    ...
    //重置密码
    services["_reset_user"] = function (param) {
        return $rootScope.serverAction(ctxPath + '/admin/user/resetPwd', param, "POST");
    }
    ...
    $scope.tableControl = {
        config: {
            ...
            columns: [{
                ...
                {field: 'dept_name', title: "所属学校", align: 'left'},
...
//重置密码
    $scope.reset = function () {
        ...
        if (ids.length == 0) {
            layer.alert('请选择重置的用户');
            return !1;
        }
        var layerPrompt=layer.prompt({title: '输入新密码，并确认', formType: 1}, function (pass, index) {
            services._reset_user({res_ids: ids, mark: pass}).success(function (res) {
                if (res.code == 0) {
                    layer.msg(res.message);

myApp.controller('levelCatController', function ($rootScope, $scope, services, $sce, $stateParams) {
    ...
    $scope.tableControl = {
        config: {
            ...
            columns: [
                {field: 'dic_code', title: "难易度编码", align: 'left'},
                {field: 'dic_name', title: "难易度名称", align: 'left'},
        ...
        reload: function (param) {
            services._dic_list(param).success(function (res) {
    ...
    $scope._form_submit = function () {
        if (!$scope.selRow.dic_code) {
            layer.alert("请填写难易度编码")
            return false;
        }
        if (!$scope.selRow.dic_name) {
            layer.alert("请填写难易度名称")
            return false;
        }
        $scope.selRow.dic_parentcode="80000";
    ...
    $scope.reload = function (key, value) {
        $scope.tableControl.config.param["dic_code"] = "80000";

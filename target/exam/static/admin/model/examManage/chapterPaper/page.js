myApp.controller('chapterPaperController', function ($rootScope, $scope, services, $sce, $stateParams, $state) {
    ...
    $scope.init = function(_call){
        ...
        services._dic_list({pageNum: 1, pageSize: 1000, dic_code: '10000'}).success(function (res) {
            ...
            $scope._gradeData = angular.copy(res.data.rows);
        ...
        services._dic_list({pageNum: 1, pageSize: 1000, dic_code: '20000'}).success(function (res) {
            ...
            $scope._specialtyData = angular.copy(res.data.rows);
    ...
    //课程选择列表
    $scope.curr_list = function () {
        //清空搜索input
        $scope.currTableControl.searchText = null
        $scope.currTableControl.gra_name = null;
        $scope.currTableControl.spe_name = null;
        $scope.openCurr(function (array) {
            angular.forEach($scope.currTableControl.rows, function (item, index) {
                if (item.select) {
                    $scope.speTest = {
                        dic_name: $scope.currTableControl.data[item.index].dic_name,
                        dic_ownerid: $scope.currTableControl.data[item.index].id
                    }
                }
            });
            if ($scope.speTest.dic_ownerid) {
                $scope.load_cha();
            }
        });
    }
